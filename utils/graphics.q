...
plotTimeSeriesSubplots:{[dt;series;label]
  subplots:plt[`:subplots][`figsize pykw (14;4)];
  fig:subplots[@;0];
  ax:subplots[@;1];
  l1:ax[`:plot_date][.util.q2pydts dt;series;`color pykw "tab:red";`linestyle pykw `solid;`markersize pykw 1];
  ax[`:set_xlabel]["Date"];
  ax[`:set_ylabel][label];
  ax[`:set_title][label," vs Date"];
  fig[`:tight_layout][];
  plt[`:show][];
  }
...
// @kind function
// @category misc
// @fileoverview Plot a heat map
// @param tbl     {num[]} The table containing data
// @param title  {string} Plot label
// @return {<} The heatmap plotted
plotHeatMap:{[tbl;title]    
  subplots:plt[`:subplots][`sharey pykw 1b;`figsize pykw (14;8)];
  fig:subplots[@;0];
  ax1:subplots[@;1];
  ax1[`:set_title] title;

  sns[`:heatmap][;`ax pykw ax1;`cmap pykw "BrBG";`annot pykw 1b] .util.tab2df[tbl];
  //sns[`:heatmap][`cmap pykw "YlGnBu"];
...
plotTimeSeries:{[dt;series;label]
  ...
  plt[`:xticks][q2pydts dt .ml.arange[0;count dt;`int$0.2*count dt]];
  plt[`:show][];
  }
